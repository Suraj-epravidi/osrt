...
function getProducts($conn, $materials, $general_color, $brand, $product_type, $price_range) {
    ...
    // Add conditions based on input values if they are not placeholders or empty
    if (!empty($materials)) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Get products based on the inputs (if no input, fetch all products)
    ...
    if (!empty($products)) {
        ...
        foreach ($products as $product) {
            ...
            echo "<img src='img/property/" . htmlspecialchars($product['image']) . "' alt='Product Image' style='width:150px;height:150px;'/>";
            echo "</div><hr>";
        }
    } else {
